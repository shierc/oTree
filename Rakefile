...
def make_menu
    ...
    wikiMenuPath = File.join("_includes", "wiki_menu.html")
    puts("generating #{wikiMenuPath}")
    open("#{wikiMenuPath}", 'w') do |wikiMenu|
        wikiMenu.puts("<ul>")
        open("#{g('wiki_source')}/_Sidebar.md", 'r').each_line do |line|
            wikiItemTitle = line.match('\[([ [:alnum:]]+)\]')[1]
            wikiItemURL = wikiItemTitle.gsub(" ", "-")
            wikiMenu.puts('<li><a class="post-link" href="{{"/wiki/%s" | prepend: site.baseurl }}">%s</a></li>' % [wikiItemURL, wikiItemTitle])
        end
        wikiMenu.puts("</ul>")
    end
end
...
task :wikisub do |t|
  ...
  command = 'git submodule add --force ' + wiki_repository + ' ' + g('wiki_source')
